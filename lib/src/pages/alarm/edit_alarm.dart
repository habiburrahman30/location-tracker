...
class AlarmEditPage extends StatefulWidget {
  const AlarmEditPage({super.key, this.alarmSettings});
...
class _AlarmEditPageState extends State<AlarmEditPage> {
  ...
  buildAlarmSettings() {
    ...
    if (staircaseFade) {
      volumeSettings = VolumeSettings.staircaseFade(
        volume: volume,
        fadeSteps: [
          VolumeFadeStep(time: Duration.zero, volume: 0),
          VolumeFadeStep(time: Duration(seconds: 15), volume: 0.03),
          VolumeFadeStep(time: Duration(seconds: 20), volume: 0.5),
          VolumeFadeStep(time: Duration(seconds: 30), volume: 1),
        ],
      );
  ...
  void saveAlarm() {
    ...
    alarm.Alarm.set(alarmSettings: buildAlarmSettings()).then((res) {
  ...
  void deleteAlarm() {
    alarm.Alarm.stop(widget.alarmSettings!.id).then((res) {
      if (res && mounted) Navigator.pop(context, true);
    });
  }
